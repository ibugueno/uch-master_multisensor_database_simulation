...
import os
import shutil
import yaml  # Importar módulo para leer YAML
import numpy as np
...
h_fov = camera_config['camera']['H-FOV']  # Horizontal Field of View
v_fov = camera_config['camera']['V-FOV']  # Vertical Field of View
d_fov = camera_config['camera']['D-FOV']  # Diagonal Field of View

# Calcular tamaño del sensor basado en FOV y lente
lens_mm = 50  # Valor fijo para la lente
sensor_width = 2 * lens_mm * math.tan(math.radians(h_fov) / 2)  # Sensor horizontal
sensor_height = 2 * lens_mm * math.tan(math.radians(v_fov) / 2)  # Sensor vertical
...
for object_class in object_classes:
    ...
    for lum_index, luminance in enumerate([1, 3, 9]):
        ...
        lum_folder_path = os.path.join(object_folder_path, f'lum{lum_index}')
