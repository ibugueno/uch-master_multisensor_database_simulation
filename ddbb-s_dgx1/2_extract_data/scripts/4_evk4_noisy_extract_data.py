...
def generate_frames(events, sensor_size, output_folder, start_index, window_size_us=10_000):
    ...
    while current_time <= max_time:
        ...
        frame_path = os.path.join(output_folder, f"image_{frame_time_ms:04d}.jpg")
...
def process_aedat_files(txt_file, aedat_prefix, image_index_prefix, output_base_dir, sensor_size):
    ...
    for relative_path in tqdm(aedat_paths, desc="Procesando AEDAT4", unit="archivo"):

        # 1) Ruta completa al .aedat4 (esto sigue igual)
        aedat_file_path = os.path.join(aedat_prefix, relative_path, "events.aedat4")

        print(f"\nProcesando: {aedat_file_path}")

        # 2) Construir la carpeta de referencia, cambiando events_* por images
        ref_relpath = relative_path.replace("events_noisy", "images") \
                                   .replace("events_clean", "images")  # por si usas ambos modelos
        reference_folder = os.path.join(image_index_prefix, ref_relpath)

        # 3) Carpeta destino para los nuevos frames (se deja igual)
        ...
        if not os.path.exists(aedat_file_path):
            print(f"[ADVERTENCIA] No existe: {aedat_file_path}. Saltando.")
            continue

        with AedatFile(aedat_file_path) as f:
            if 'events' in f.names:
                events_np = np.hstack([packet for packet in f['events'].numpy()])
                events = [
                    {"x": x, "y": y, "timestamp": t}
                    for x, y, t in zip(events_np['x'], events_np['y'], events_np['timestamp'])
                ]
            else:
...
if __name__ == "__main__":
    ...
    aedat_prefix = f"../input/events/"
    image_index_prefix = f"../input/frames/"
    output_base_dir = f"../input/events-frames/"
