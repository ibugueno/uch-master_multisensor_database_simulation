...
ROOT_INPUT_ABS="/home/ignacio.bugueno/cachefs/datasets/processed_data/moving6dpose/ddbb-s/frames/${SENSOR}/images"
ROOT_OUTPUT_ABS="/home/ignacio.bugueno/cachefs/datasets/processed_data/moving6dpose/ddbb-s/frames/txt"
    ...
    for object_dir in "$SCENE_PATH"/*/; do
        for orientation_dir in "$object_dir"orientation_*/; do
            if [ -d "$orientation_dir" ]; then
                ...
                echo "$SENSOR/images/$REL_PATH" >> "$OUTPUT_FILE"
            fi
        done
    done
