...
INPUT_FILE="input/txt/folders_evk4.txt"
...
# Filtros
SCENE_FILTER="scn1"
OBJECT_FILTER="pelota" #pelota, mobil, sombrero, avion, tarro
ORIENTATION_FILTER="orientation_88_-6_-34"
...
while IFS= read -r line; do
    ...
    # Filtrar por el objeto específico
    if [[ "$line" != *"$OBJECT_FILTER"* ]]; then
        echo "El directorio no contiene el objeto filtrado ($OBJECT_FILTER): $line"
        continue
    fi

    # Filtrar por la orientación específica
    if [[ "$line" != *"$ORIENTATION_FILTER"* ]]; then
        echo "El directorio no contiene la orientación filtrada ($ORIENTATION_FILTER): $line"
        continue
    fi
