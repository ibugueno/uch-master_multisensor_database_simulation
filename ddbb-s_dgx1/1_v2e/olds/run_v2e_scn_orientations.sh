...
INPUT_FILE="input/txt/evk4_generate_val_with_back_without_blur.txt"
...
while IFS= read -r line; do
    ...
    # Procesar solo las líneas que contienen "scn2"
    if [[ "$line" != *"scn2"* ]]; then
        continue
    fi
    ...
    for model in clean; do
        ...
        output_folder="output/aedat_val_with_back_without_blur/$model/$line"

        # Ejecutar el comando
        python v2e.py -i "input/frames_for_generate_val_with_back_without_blur/3_ddbs-s_with_back_without_blur/$line" \
            --overwrite \
