...
def main():
    ...
    # --- Set torch device from args ---
    if args.use_cuda and torch.cuda.is_available():
        torch_device = torch.device(f"cuda:{args.gpu_id}")
        logger.info(f"Using CUDA on GPU {args.gpu_id}: {torch.cuda.get_device_name(torch_device)}")
    else:
        torch_device = torch.device("cpu")
        logger.warning("Using CPU (CUDA disabled or not available)")
