...
# Dividir la lista de clases en segmentos
segment_size = len(object_classes) // num_segments
segments = [object_classes[i:i + segment_size] for i in range(0, len(object_classes), segment_size)]

# Ruta base para los archivos de scripts
base_script_path = "/data/codes/"

# Crear y ejecutar los scripts de renderizado en paralelo
for i, segment in enumerate(segments):
    script_path = f"{base_script_path}render_segment_{i}.py"
...
object_classes = {segment}

base_path = "../input/all/"
output_base_folder_path = "../output/output_frames/segment_{i}"
...
for object_class in object_classes:
    ...
    # Mover configuración de cámara después de importar el objeto
    camera_object = bpy.context.scene.objects.get("Camera")
    if camera_object:
...
""")
    ...
    command = f"conda run -n blender python {script_path}"
    subprocess.Popen(command, shell=True)

print("Renderización de todos los segmentos iniciada.")
